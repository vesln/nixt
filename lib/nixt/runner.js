...
function Runner(options) {
  ...
  this.formatter = new Formatter({ colors: options.colors, newlines: options.newlines });
  this.expectations = [];
...
Runner.prototype.run = function(cmd, fn) {
  this.command().set(this.baseCmd + cmd);
  this.batch.main(this.execFn());
  if (fn) this.end(fn);
  return this;
};
...
Runner.prototype.exec = function(cmd) {
  this.batch.add(middlewares.exec(cmd, this._cwd));
  return this;
};
...
Runner.prototype.end = function(fn) {
  if (!this.batch.hasMain()) throw new Error('Please provide a command to run. Hint: `nixt#run`');
  this.batch.run(fn);
};
...
Runner.prototype.command = function() {
  this._command = this._command || new Command(this.formatter);
...
Runner.prototype.execFn = function() {
  var self = this;

  return function(fn) {
    self.command().exec(function(result) {
      var err = null;
      for (var i = 0, len = self.expectations.length; i < len; i++) {
        err = self.expectations[i](result);
        if (err) break;
      }
      fn(err);
    });
